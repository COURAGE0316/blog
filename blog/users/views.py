...
class RegisterView(View):
    ...
    def post(self,request):
        ...
        print('从redis中获取的验证码：',redis_sms_code.decode())
...
class ImageCodeView(View):
    def get(self,request):
        ...
        :return:
        '''
        # 1.接收前端传递过来的uuid（uuid是前端所生成的（唯一））
...
class SmsCodeView(View):
    def get(self,request):
        ...
        print('redis_image_code:',redis_image_code.decode)
...
# Django用户认证系统提供了方法
# 选择第二种：LoginRequiredMixin封装了判断用户是否登录的操作
# 在当前类中继承这个类，然后进行http://127.0.0.1:8000/center/访问
# 如果用户未登录的话，则会进行默认的跳转
# 默认的跳转链接：accounts/login/?next=xxx（/center/）
# 可以在setting中修改系统的默认跳转
# 实现next参数跳转到我们指定的页面的业务（到视图的登录业务逻辑里面来）
...
class UserCenterView(LoginRequiredMixin,View):
    ...
    # 用户点击修改按钮的时候是进行一个post请求
    def post(self,request):
        '''
        # 1.接收参数
        # 2.将参数保存起来（用户名和简介信息可以直接保存到数据库中，头像信息是保存的头像的路径，将头像信息保存到指定文件目录中）
        # 3.更新cookie信息
        # 4.刷新当前页面（重定向操作）
        # 5.返回响应
        :param request:
